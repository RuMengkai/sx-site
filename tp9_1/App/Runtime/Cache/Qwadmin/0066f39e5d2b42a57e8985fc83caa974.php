							...
							<div class="col-xs-12">
								...
								<form id="form" method="post" action="<?php echo U('del');?>">
									...
									<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr>
											<td class="center"><input class="mids" type="checkbox" name="mids[]" value="<?php echo ($val['mid']); ?>"></td>
											<td><?php echo ($val['title']); ?></td>
											<td><?php echo ($val['content']); ?></td>
											<td><?php echo ($val['user']); ?></td>
											<td><?php echo (date("Y-m-d H:i:s",$val['dateline'])); ?></td>
											<td><a href="<?php echo U(edit);?>?mid=<?php echo ($val['mid']); ?>"><i class="ace-icon fa fa-pencil bigger-100"></i>修改</a>&nbsp&nbsp;<a href="javascript:;" val="<?php echo U('del');?>?mids=<?php echo ($val['mid']); ?>" class="del"><i class="ace-icon fa fa-trash-o bigger-100 red"></i>删除</a></td>
...
<![endif]-->
		...
		$(function(){
			$(".check-all").click(function(){
				$(".mids").prop("checked", this.checked);
			});
			$(".mids").click(function(){
				var option = $(".ids");
